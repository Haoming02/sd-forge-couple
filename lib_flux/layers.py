"""
Credit: logtd
https://github.com/logtd/ComfyUI-Fluxtapoz/blob/main/flux/layers.py
"""
...
def ref_attention(q, k, v, pe, ref_config, ref_type, idx, txt_shape=256):
    ...
    attn_a = attention(
        q[:1], k2, v2, pe=pe[:1], k_pe=torch.cat([pe[:1], ref_pes[0]], dim=2)
    )
...
class DoubleStreamBlock(OriginalDoubleStreamBlock):
    def forward(
        ...
        mask = None if mask_fn is None else mask_fn(q, transformer_options, 256)

        rfedit = transformer_options.get("rfedit", {})
        if (
            rfedit.get("process", None) is not None
            and rfedit["double_layers"][str(self.idx)]
        ):
            pred = rfedit["pred"]
            step = rfedit["step"]
        ...
        if (
            ...
            and ref_config["strengths"][ref_config["step"]] > 0
            and self.idx <= 20
        ):
            attn = ref_attention(q, k, v, pe, ref_config, "double", self.idx)
        elif rave_options:
            attn = rave_rope_attention(
                img_q,
                img_k,
                img_v,
                txt_q,
...
class SingleStreamBlock(OriginalSingleStreamBlock):
    def forward(
        ...
        if (
            rfedit.get("process", None) is not None
            and rfedit["single_layers"][str(self.idx)]
        ):
            pred = rfedit["pred"]
            step = rfedit["step"]
            if rfedit["process"] == "forward":
        ...
        if (
            ref_config is not None
            and ref_config["single_strength"] > 0
            and self.idx < 10
        ):
