...
DEFAULT_MAPPING = [["0:0.5", "0.0:1.0", "1.0"], ["0.5:1.0", "0.0:1.0", "1.0"]]
...
def visualize_mapping(p_WIDTH: int, p_HEIGHT: int, data: list) -> Image:
    ...
    for tile_index in range(len(mapping)):
        ...
        draw.rectangle(
            [(x_from, y_from), (x_to, y_to)], outline=COLORS[color_index], width=4
        )
...
def couple_UI(script, is_img2img: bool, title: str):
    with gr.Accordion(
        label=title,
        elem_id=f"forge_couple_{'i2i' if is_img2img else 't2i'}",
        open=False,
    ):
        with gr.Row():
        ...
        with gr.Group(visible=False, elem_classes="fc_adv") as adv_settings:
            ...
            with gr.Row(elem_classes="fc_map_btns"):
                with gr.Column():
                    add_first = gr.Button("New First Row")
                    add_first.click(add_first_row, mapping, mapping)
                    add_last = gr.Button("New Last Row")
                    add_last.click(add_last_row, mapping, mapping)
                with gr.Column():
                    del_first = gr.Button("Delete First Row")
                    del_first.click(del_first_row, mapping, mapping)
                    del_last = gr.Button("Delete Last Row")
                    del_last.click(del_last_row, mapping, mapping)

                reset_map = gr.Button("Reset Mapping")
                reset_map.click(reset_mapping, None, mapping)
